...
// TypeScript implementation by Aevarkan
// Changes:
...
export class ItemDatabase {
     ...
     */
    private static readonly STORAGE_ENTITY: string = "areas:item_storage"
    /**
     ...
     * @param namespace The unique namespace for the database keys. This will be the prefix used before the `:` in the structure's name.
     * 
     * @param cacheSize The max amount of keys to keep quickly accessible. A small size can cause lag on frequent iterated usage, a large number can cause high hardware RAM usage.
     * 
     *                  The default size is 50 elements.
     * 
     * @param saveRate The background saves per tick, (high performance impact)
     * 
        ...
        system.run(() => {
            ...
            if (player) {
                if (!sl || sl?.hasParticipant('x') === false) {
                    if (!sl) sl = world.scoreboard.addObjective(ItemDatabase.SCOREBOARD_ID);
                    sl.setScore('x', player.location.x)
                    sl.setScore('z', player.location.z)
                    this.spawnLocation = { x: sl.getScore('x'), y: 318, z: sl.getScore('z') }
     ...
     * @param requiredEntities The number of entities required to contain all inventories. Each entity can store a maximum of 256 slots.
     * 
     ...
     * @remarks This spawns in empty entities if the value for the key doesn't exist.
     * 
    ...
    private getInventories(key: string, requiredEntities?: number) {
        ...
        if (structure) {
            ...
            for (let i = 0; i < requiredEntities; i++) {
                this.dimension.spawnEntity<typeof ItemDatabase.STORAGE_ENTITY>(ItemDatabase.STORAGE_ENTITY, this.spawnLocation)
            }
        }

        // Now get those storage entities
        const entities: Entity[] = this.dimension.getEntities({ location: this.spawnLocation, type: ItemDatabase.STORAGE_ENTITY })
        if (requiredEntities) {
            // Spawn more storage entities if needed
            if (entities.length < requiredEntities) {
                for (let i = entities.length; i < requiredEntities; i++) {
                    entities.push(this.dimension.spawnEntity<typeof ItemDatabase.STORAGE_ENTITY>(ItemDatabase.STORAGE_ENTITY, this.spawnLocation))
                }
            }
    ...
    private async saveStructure(key: string, existingStructure: boolean) {
        ...
        const entities = this.dimension.getEntities({ location: this.spawnLocation, type: ItemDatabase.STORAGE_ENTITY });
     ...
     * @throws Throws if `value` is an array that has more than 1024 items.
     * @remarks This function can be called in read-only mode (That's not a typo). 
     * 
     *          The maximum array size is 1024 elements.
     */
    public set(key: string, value: ItemStack[] | ItemStack) {
        const time = Date.now();
     ...
     * @returns All the values as an array of ItemStack[].
     * @remarks This function can't be called in read-only mode.
     */
